...
class Format(General):
    ...
    def output_extract(self):
        output('static void ', self.extract_name(), '(',
               'inst_info', ' *a, ', insntype, ' insn)\n{\n')
        # output('    strcpy((char *)(a), "', self.opcode, '");\n')
        output('    a->op_count', ' = ', str(len(self.fields)), ';\n')
        index = 0
        for n, f in self.fields.items():
            output('    strcpy((char *)(a->operands[', str(index), '].name), "', n, '");\n')
            output('    a->operands[', str(index), '].value', ' = ', f.str_extract(), ';\n')
            index += 1
        
        output('    a->src_reg_cnt', ' = ', str(len(self.fields)), ';\n')
        output('    a->dst_reg_cnt', ' = ', str(len(self.fields)), ';\n')
        output('}\n\n')
# end Format
...
class Pattern(General):
    ...
    def output_code(self, i, extracted, outerbits, outermask):
        ...
        if len(self.fields) > 0:
            output(ind, 'index = u->op_count;\n');
        for n, f in self.fields.items():
            output(ind, 'strcpy((char *)(u->operands[index].name), "', n, '");\n')
            output(ind, 'u->operands[index].value', ' = ', f.str_extract(), ';\n')
            output(ind, 'index++;\n')
...
def main():
    ...
    # insist that decode begins from naught. // anught = nothing
    for i in toppat.pats:  # MultiPattern中的prop_mask是重新计算mask 包括fixedbits，fixedmask，undefmask
        i.prop_masks()
    ...
    # output_autogen()  # 输出 /* This file is autogenerated by scripts/decodetree.py.  */
#     output('''
# typedef struct {
#     char name[16];  // 操作数名称
#     int value;      // 操作数数值 
...
# ''')
    ...
    output(decode_scope, 'inst_info* ', decode_function,
           '(', insntype, ' insn)\n{\n')

    i4 = str_indent(4)

    if len(allpatterns) != 0:
        output(i4, "inst_info *u = (inst_info*)malloc(sizeof(inst_info));\n")
        output(i4, 'u->size = ', str(insnwidth // 8), ';\n');
        output(i4, 'int index;\n\n');
